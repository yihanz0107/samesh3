...
from data.loaders import remove_texture
from renderer.renderer import Renderer, render_multiview, colormap_faces, colormap_norms
from models.sam import Sam3Model,combine_bmasks, colormap_mask, remove_artifacts
from utils.cameras import *
from utils.mesh import duplicate_verts
from models.shape_diameter_function import *
...
class SamModelMesh(nn.Module):
    ...
    def __init__(self, config: OmegaConf, device='cuda', use_sam=True):
        ...
        if use_sam and (self.config.cache is None or not self.config.cache.exists() or self.config.cache_overwrite):
            self.sam = Sam3Model(config, device=device)
